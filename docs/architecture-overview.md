...
2026-09-05 本地验证：前端 67 项会话、插件、导航、宿主和 provider 回归通过；另有真实 Element Plus 依赖加载检查。Registry/Runtime/导航模块行覆盖率为 99.23%，架构检查器 17 项测试通过、行覆盖率为 93.43%。后端全量 `go test -race ./...`、`go vet ./...`、`go build ./...` 通过。
